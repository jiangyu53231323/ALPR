...
# 首先是三个模块，BasicBlock 和 Bottleneck 和 ResNet 中的一致，BottleneckX 实际上
# 是 ResNeXt 中的基础模块，也可以作为 DLA 中的基础模块。DLA34 中调用的依然是 BasicBlock。
...
# 所有的 Aggregation Node 都是通过调用这个模块完成的，这个绿色结点也是其连接两个树的根，所
# 以形象地称之为 Root。下面是代码实现，forward 函数中接受的是多个对象，用来聚合多个层的信息。
...
# Tree 类对应图中的 HDA 模块，是最核心最复杂的地方。其核心就是递归调用的 Tree 类的构建
...
# Tree 是 DLA 最重要的模块，Tree 搞定之后，DLA 就按顺序拼装即可
class DLA(nn.Module):
    '''
    DLA([1, 1, 1, 2, 2, 1],
...
# DLASeg 是在 DLA 的基础上使用 Deformable Convolution 和 Upsample 层组合进行信息提取，提升了空间分辨率。
